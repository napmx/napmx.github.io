...
서버(media-conf)의 광고 단위 `interval`(초)이 **0보다 클 때만** 노출 후 `interval`마다 자동 갱신되고, 실패 시 재요청합니다(최소 5초 간격). `interval`이 0(미설정)이면 단발성(자동 재로드 없음)입니다. 무한 실패-재로드는 SDK 루프 가드(#59)가 자동 차단합니다. *(v2.0.0: 기존 `isRetry`/`maxRetryCountInSlot` 옵션 제거 — 서버 `interval` + 내부 가드로 일원화)*
