...
광고 네트워크·소재 유형에 따라 실제 노출 사이즈가 달라질 수 있어 설정 사이즈가 보장되지 않습니다. 컨테이너를 **너비 `match_parent` + 높이 `wrap_content`** 로 두면 소재 크기에 맞게 조정됩니다. **AdManager 배너는 서버 광고단위에 등록된 너비와 높이로 요청됩니다.**
...
서버 소재에 선택(optional) asset이 없으면 **미디에이션 어댑터(Naver·Pangle·AdManager 등)** 가 해당 View를 빈 칸이 아니라 **자동으로 `GONE` 처리**합니다. 단 **AdMixer 자체 소재는 자동 숨김 대상이 아니므로**(빈 값이 그대로 설정되고 CTA는 기본 문구로 대체) 누락 케이스 레이아웃을 함께 확인하세요. 모든 asset View를 선언해 두어도 소재에 없는 항목은 표시되지 않으니, 누락 케이스에서도 레이아웃이 자연스럽도록 상대 위치 제약(`layout_below` 등)으로 배치하세요. `title`/`icon`/`mainView` 중 최소 1개는 반드시 사용해야 합니다.
...
**Q. 종료 Dialog/BottomSheet에 배너나 네이티브 광고를 미리 로드할 수 있나요?**

네. **같은 홈 Activity가 생성과 표시를 모두 담당하는 경우** Activity Context로 광고
뷰를 직접 생성해 부모에 붙이지 않은 채 로드하고, 팝업을 열 때 전용 컨테이너에
붙이면 됩니다. 기존 부모가 있다면 `addView()` 전에 반드시 선분리하고, 앱에서
신선도 기준(권장 30분)과 `stop()` 해제를 관리하세요.

Unity 배너가 포함된 지면은 로드 시점에 네트워크 임프레션이 잡힐 수 있어 엄격한
팝업 노출 시점 보장이 필요하면 Unity를 제외해야 합니다. 전체 예제는
[배너 가이드](banner.md)와 [네이티브 가이드](native-ad.md)를 참고하세요.
