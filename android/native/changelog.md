...
### Deprecated (동작 변경 없음 — 차기 메이저에서 제거)

- `AdInfo.Builder.interstitialTimeout(int)` — 동작하지 않는 옵션(로드 타임아웃은 서버 설정을 따름)
- `AdInfo.Builder.setUseBackgroundAlpha(boolean)` — 동작하지 않는 옵션(전면 배경 디밍은 자동 적용)
- `AdMixer.AX_ERR_NO_FILL` — SDK가 전달하지 않는 예약 상수. no-fill 분기는 `AX_ERR_NO_ADS`를 사용하세요 ([에러 코드](error-codes.md))
