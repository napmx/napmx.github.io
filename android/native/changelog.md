...
> | 아티팩트 | 버전 |
> |---|---|
> | `admixer-ssp` (코어) | **2.3.0** |
> | `admixer-admanager` · `admixer-naveradmanager` | **2.1.3** |
> | `admixer-gma-nextgen` · `admixer-pangle` · `admixer-teads` | **2.1.2** |
> | `admixer-adfit` · `admixer-unity` | **2.0.6** |
> | `admixer-applovin` | **2.0.5** |
...
> 코어는 v2.2.2 그대로이며 아래 어댑터만 배포됩니다. 이 릴리스 시점의 BOM은 `admixer-bom:2026.09.02`입니다.
>
> | 아티팩트 | 버전 |
> |---|---|
> | `admixer-admanager` | **2.1.2** |
> | `admixer-naveradmanager` | **2.1.2** |
> | `admixer-bom` | **2026.09.02** |
> | 그 외 어댑터·모듈 전체 | 변경 없음 (v2.2.2 표 참고) |

### 안정성 개선 (코드 변경 불요)

- **AdManager 배너 요청 크기 정정** — 서버에 등록된 지면 크기(예: 320×100)를 그대로 GAM에 요청합니다.
...
- **전화 상태 권한 정책 정비 (#167)** — 미사용 `READ_PHONE_NUMBERS` 등 불필요한 권한
  선언을 제거했습니다. 통신 세대 판별은 Android 13+에서 `READ_BASIC_PHONE_STATE`,
  Android 12 이하는 앱이 `READ_PHONE_STATE`를 허용한 경우에만 수행하며, 권한이 없으면
  기존처럼 `3G`로 폴백합니다. 병합 매니페스트의 권한 목록이 달라질 수 있으니 확인하세요.
- **어댑터 로드 타임아웃 오류 코드 정정** — 어댑터 자체 타임아웃이 네트워크 오류 대신
  `AX_ERR_TIMEOUT`으로 보고됩니다. 오류 코드로 분기하는 앱은 확인하세요.

### 개인정보 보호 개선 (코드 변경 불요)

- **설치 앱 목록 수집 차단 (#134)** — COPPA 아동 대상 또는 GDPR 명시적 거부 상태에서는
  설치 앱 목록을 수집하지 않으며, 로그에서도 마스킹됩니다.
  ([개인정보 가이드](privacy.md#설치-앱-목록-수집-정책))
- **전면 광고 신고 캡처 최소화 (#136)** — 사용자가 신고 아이콘을 누른 경우에만 화면을
  캡처합니다.
