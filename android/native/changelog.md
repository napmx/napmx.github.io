...
- **전면 BACK 키 기본 차단** — `AdInfo.Builder.setDisableBackKey` 기본값 `true`(전면·비디오·리워드 전 풀스크린 공통). 종전처럼 BACK으로 닫으려면 `false` 명시. ([Step 8](migration.md))
...
- **AdMixer 단독 네이티브 광고 설명(description) 노출 오류 수정** — 코어 네이티브 뷰(AXAdNativeView)에서 본문 설명(description, 103) 텍스트 뷰 바인딩 누락 결함을 수정했습니다.
